...
WPMsg.ready(() => { // .then() : démarre toujours APRÈS l'évaluation complète du script
  ...
  // Palette des PIÈCES ADVERSES (/pa) : titre, invite et pastilles dorées propres.
  const adv = params.get("side") === "adv";
  if (adv) {
    document.title = "Citer une pièce adverse";
    document.body.classList.add("adv");
    q.placeholder = "Pièce adverse : nom ou numéro…";
  }
  ...
  function choose(i) {
    const p = filtered[i];
    if (p) WPMsg.send(JSON.stringify({ id: p.id }));
  }
  function cancel() { WPMsg.send(JSON.stringify({ cancelled: true })); }
  document.getElementById("close").addEventListener("click", cancel); // croix : seul moyen SOURIS d'annuler
