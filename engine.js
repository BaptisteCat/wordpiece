...
(function () {
  ...
  function defaultModel() {
    return { version: 3, settings: defaultSettings(), pieces: [], ignored: [] };
  }
  ...
  // Découpe un tag « wp:piece:<id>[#<complément>] ».
  function splitPieceTag(tag) {
    const raw = String(tag || "").slice(TAG_PREFIX.length);
    const i = raw.indexOf(CITE_EXTRA_SEP);
    return i < 0 ? { id: raw, extra: "" } : { id: raw.slice(0, i), extra: raw.slice(i + 1) };
  }
  function pieceIdFromTag(tag) { return splitPieceTag(tag).id; }
  function pieceTagFor(id, extra) { return TAG_PREFIX + id + (extra ? CITE_EXTRA_SEP + extra : ""); }
  // Le tag désigne-t-il une citation de CETTE pièce (avec ou sans complément) ?
  function isTagOfPiece(tag, id) {
    return !!tag && tag.indexOf(TAG_PREFIX) === 0 && splitPieceTag(tag).id === id;
  }
    ...
    await runUntracked(async (context) => {
      ...
      for (const cc of pieceCCs) {
        ...
        if (piece) {
          ...
          writeCite(cc, formatCitation(num, piece.name) + extra, "Pièce " + num);
        } else markOrphan(cc);
      }
  ...
  async function getPieceIdAtSelection() {
    ...
    await Word.run(async (context) => {
      ...
      await context.sync();
      if (!cc.isNullObject && cc.tag && cc.tag.startsWith(TAG_PREFIX)) id = pieceIdFromTag(cc.tag);
    });
    return id;
  }
  ...
  async function deletePieceEverywhere(id) {
    ...
    const removeIds = [id];
    if (target && target.container) for (const kid of childrenOf(id)) removeIds.push(kid.id);
    await runUntracked(async (context) => {
      const ccs = context.document.body.contentControls;
      ccs.load("items/tag,items/id");
      await context.sync();
      const matching = ccs.items.filter((cc) => removeIds.some((rid) => isTagOfPiece(cc.tag, rid)));
      const paras = matching.map((cc) => cc.getRange("Whole").paragraphs.getFirst());
  ...
  async function finalizeFormat(pending) {
    ...
    await runUntracked(async (context) => {
      ...
      if (bm.isNullObject) { await finish(); return; } // ligne /pf effacée : rien à reprendre
      ...
      if (!after) {
        if (rel.value === "Before" || rel.value === "AdjacentBefore") await finish(); // curseur remonté
  ...
  async function setNumber(id, value) {
    ...
    if (v === "") { p.fixedNumber = null; p.locked = false; } // coulant
    else {
      const r = placeAt(p, v);
      if (r && r.error) return r;
    }
    await save();
    return { ok: true, num: computeStructuredNumbers().get(p.id) };
  }
  ...
  async function setCitationExtra(ccId, extra) {
    await runUntracked(async (context) => {
      ...
      await context.sync();
      if (cc.isNullObject || !cc.tag || cc.tag.indexOf(TAG_PREFIX) !== 0) return;
      cc.tag = pieceTagFor(splitPieceTag(cc.tag).id, extra || "");
      await context.sync();
    });
  }
  ...
  window.WP = {
    ...
    isWarningIgnored, ignoreWarning, restoreWarnings, ignoredCount,
